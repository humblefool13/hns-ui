...
export async function GET() {
  try {
    ...
    while (true) {
      try {
        const tld = (await contract.read.registeredTLDs([
          BigInt(index)
        ])) as string;
        if (tld && tld !== "") {
          tlds.push(tld);
    ...
    const contracts: { tld: string; contract: Address }[] = [];
    for (const tld of tlds) {
      const TLDContract: Address = await contract.read.tldContracts([tld]);
      contracts.push({ tld, contract: TLDContract });
    }
