...
import { Globe, Clock, Settings, RefreshCw, Plus, Trash2, ExternalLink, Lock, Unlock } from 'lucide-react';
        ...
        <div className="glass-card p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-xl">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300">Total Names</div>
            </div>
            ...
            <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
              <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400 mb-1">1</div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-foreground font-mono">{domain.name}</h3>
                      <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          domain.status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
                          domain.status === 'expiring' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200' :
                          'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'
                        }`}>
                          {domain.status === 'active' ? 'Active' : domain.status === 'expiring' ? 'Expiring Soon' : 'Expired'}
                        </span>
                  ...
                  <div className="flex items-center gap-3">
                    <button className={`p-2 rounded-lg transition-all duration-300 ${
                      ...
                      {domain.isLocked ? <Lock size={16} /> : <Unlock size={16} />}
                    </button>
